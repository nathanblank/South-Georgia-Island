...
#After the number, an ERA-5 absoulte pathname is printed on the same line [2:61], then the map number of that ERA-5 file [62:66], then a date and time [67:86], then an average wind speed [88:100], then an average wind direction [102:106]
...
ERA_5_in = '/Volumes/Blanken_HD/ERA5_Output_Data/ERA5_mnspd_mndir_MASTER.txt' #ERA-5 master file
Combined_master_output = '/Volumes/Blanken_HD/Combined_ERA_5_and_QuikSCAT_Master.txt' #Output file
...
ERA_5_line_number = 1 #initializes line 0 for counting through ERA-5 files
QuikSCAT_writing_array = ['1', '2', '3', '4'] #makes array for absolute QuikSCAT path names to be written into
last_ERA_5_line = '/Volumes/Blanken_HD/ERA5_Data/ERA5_downloads/2000_01_uv.nc, 001, 2000-01-01 06:00:00, 10, 349'
hour_diff_in_QuikSCAT_and_ERA_5 = 4 #sets the initial difference in hours to be greater than 3 so that the program goes into the while loop
...
for l in range(1, len(all_QuikSCAT_lines)): #for loop going through all QuikSCAT files
    writing_array_element = 0 #setting the QuikSCAT writing array element to 0

    while hour_diff_in_QuikSCAT_and_ERA_5 > 3: #while loop checking that the difference in hours between an ERA-5 map and QuikSCAT rev is greater than 3

        current_ERA_5_line = all_ERA_5_lines[ERA_5_line_number] #grabs the current ERA-5 line (pathname, time, etc.)
        current_ERA_5_time = current_ERA_5_line[65:84] #pulls out the time
        current_QuikSCAT_line = all_QuikSCAT_lines[l] #pulls current QuikSCAT line
        ...
        if hour_diff_in_QuikSCAT_and_ERA_5 <= 3.0: #checks to see if it found a QuiKSCAT time that matches with an ERA-5 time
            ...
            print('hit')
        if hour_diff_in_QuikSCAT_and_ERA_5 > 3.0: #if not hitting the QuikSCAT file, next ERA-5 map is pulled
            ERA_5_line_number += 1


    if last_ERA_5_line != current_ERA_5_line: #if it is a new ERA-5 file
        difference_in_lines = l - line_counter #find the difference in number of QuikSCAT lines from when last ERA-5 file was written
        line_counter = l #set the new line value
        print(difference_in_lines, line_counter)
        Read_output_master.write(str(difference_in_lines) + ' ' + str(last_ERA_5_line)) #write the current ERA-5 line
        print('QuikSCAT rev counter: ' + str(difference_in_lines))

        last_ERA_5_line = current_ERA_5_line #set new ERA-5 line
    ...
    hour_diff_in_QuikSCAT_and_ERA_5 = 4 #makes sure that the program goes back into for loop upon new QuikSCAT line