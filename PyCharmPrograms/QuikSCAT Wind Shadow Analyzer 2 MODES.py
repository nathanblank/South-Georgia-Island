#This program outputs characteristics of a wind shadow using the 2 mode method (directly behind island and then some distance down wind).
# It uses ERA-5 to calculate the average upwind speed, not a set constant.
import numpy as np
...
for l in range(0,len(all_lines)):
    ...
    if 8 <= ERA_5_wind_speed <= 10 and (300 <= ERA_5_wind_direction <= 359 or 0 <= ERA_5_wind_direction <= 30): #checking if the ERA-5 map fits the conditions
                            ...
                            if lat_Array[r, c] < -45.3:#checking for values that meet lat range
                                if lat_Array[r, c] > -63.3:
                                    if flags_Array[r,c] == 0:
                                        ...
                                        #print()
                                        #print()
        #This set of loops is to average out any wind speeds from revs that overlap one another
        ...
        #This set of loops is to "smooth" out the plot so that way there are no small 0.0 wind speeds due to resolution
        ...
        if np.any(CALCULATED_current_revs[current_row-5:current_row+6,current_col] <= (.9*up_map_speed)) == True and np.sum(CALCULATED_current_revs[94:104, current_col]) != 0.0: #to check and see if there is a wind shadow directly behind SGI
            print("***************")
            print("WIND SHADOW PRESENT DIRECTLY BEHIND ISLAND")
            print('***************')
            countable_wind_shadows += 1
            directly_behind_shadow += 1
        ...
        elif np.any(CALCULATED_current_revs[current_row-5:current_row+6,current_col+5] <= (.9*up_map_speed)) == True and np.sum(CALCULATED_current_revs[94:104, current_col+5]) != 0.0: #checks to see if there is a wind shadow downwind of the island
            current_col += 5 #accounts for 50 km shift
            print("***************")
            print("WIND SHADOW PRESENT 130 KM DOWNWIND")
            print('***************')
            countable_wind_shadows += 1
            downwind_shadow += 1
            total_wind_shadows += 1
            ...
            print("Starting row: " + str(current_row))
            print("Starting col: " + str(current_col))
        else: #no wind shadow found
            print("NO VALID WIND SHADOW")
            for r in range(-5,6): #determines where start of wind shdaow is based on moving up and down 50 km each (100 km total)
                print(r,CALCULATED_current_revs[current_row+r,current_col-10],CALCULATED_current_revs[current_row+r,current_col], (.9*up_map_speed))
        ...
        while CALCULATED_current_revs[current_row, current_col] <= (up_map_speed*.9) and CALCULATED_current_revs[current_row,current_col] != 0.0 and current_row < 199 and current_row > 0 : #for each column
            ...
            while CALCULATED_current_revs[current_row, current_col] <= (up_map_speed*.9) and CALCULATED_current_revs[current_row,current_col] != 0.0 and current_row < 199 and current_row > 0 : #loop for moving down in rows
                ...
                if CALCULATED_current_revs[current_row,current_col] < lowest_speed_in_last_column: #finding lowest wind speed in column
            ...
            if (bottom_of_col - top_of_col) < 4 and (current_col-109) > 3: #checking 40 km condition
        ...
        length_of_wind_shadow = np.sqrt((((abs(current_row - 99))/lat_resolution)**2) + (((abs(current_col - 109))/long_resolution) ** 2)) #determining length of wind shadow
...
print("Wind shadows directly behind island: " + str(directly_behind_shadow))
print("Wind shadows 50 km from island: " + str(downwind_shadow))
...
plt.title(('QuikSCAT data 2 MODE FINDER' + '\n' + '8-10 m/s and 300-30 degrees NEW DATA 50 KM'), fontdict=None, loc='center', pad=None, fontsize = 32, **TNRfont)
print('QuikSCAT data 2 MODE FINDER' + '\n' + '8-10 m/s and 300-30 degrees NEW DATA 50 KM')
