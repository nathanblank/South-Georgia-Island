#This program allows the user to put in a date and time, selects that map, and then creates plot of that map.
#The time must be in the range of the input file
import numpy as np
...
fline = '/Volumes/Satellite_Data/ERA5_downloads/2005_01_uv.nc'     # This is a sample absolute address
#fline = '/Users/N-Dogg/Downloads/SRT IV/ECMWF_ERA5_Analyses/2005_01_uv.nc'
...
def ERA5MapNumberDeterminer(human_year,human_month,human_day,human_hour,min,sec):
    ...
    hoursSinceTrueStart = (time_difference*24) + human_hour
    print(hoursSinceTrueStart)
    if hoursSinceTrueStart > (len(w10)-1) and hoursSinceTrueStart < w10[0]:
...
mapNumBegin = ERA5MapNumberDeterminer(human_year,human_month,human_day,human_hour,0,0) #mapNumBegin is the map number you're looking at
mapNumEnd = mapNumBegin # last map number
...
plt.title(('Average Wind Speeds From ' + str(datetime(human_year,human_month,human_day,human_hour,min,sec)) +' to '+ str(datetime(human_year,human_month,human_day,human_hour,min,sec)) + ' in m/s'), fontdict=None, loc='center', pad=None)
