    ...
    if 8 <= ERA_5_wind_speed <= 10 and (300 <= ERA_5_wind_direction <= 359 or 0 <= ERA_5_wind_direction <= 30): #checking if the ERA-5 map fits the conditions
        ...
        for r in range(rowBegin, rowEnd + 1):  # goes through all of the ERA-5 range that covers 2000 km plot
            for c in range(colBegin, colEnd + 1):
                row_index, col_index = sub.Two_thousand_km_index_finder(latArray[r], longArray[
                    c])  # converts coordinate to 2000 km plot
        ...
        for r in range(90, 110):
            for c in range(40, 90):
                if current_ERA_5_2000_km[
                    r, c] != 0:  # checks for only ERA-5 elements that are non-zero. Some are 0 because of ERA-5 to 2000 km resolution change
...
output_file = '/Volumes/Blanken_HD/NEW_8_10_and_300_30_QuikSCAT_ACCEPTED.txt'
