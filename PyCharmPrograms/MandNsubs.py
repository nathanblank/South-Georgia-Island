...
#def twelfth_to_fourth_rounder(remainder):
   # print(remainder)
   # switcher = {
    #    0.0000: 0,
     #   0.0833: 0,
      ...
      #  0.5000: 2,
       # 0.5833: 2,
        ...
        #0.9166: 4,
    #}
    #print("Switched value: " + str(switcher.get(remainder)))
    #return switcher.get(float(remainder))
...
def ETOPO5_five_coordinate_finder(row, col):
    ...
    global row_coord
    global col_coord
    global shifted_row_coord
    global shifted_col_coord
    global center_row_coord
    global center_col_coord
    ...
    row_coord = "{0:.4f}".format(row_coord)
    ...
    col_coord = "{0:.4f}".format(col_coord)
...
# ERA5 (720,1440) - this program takes in an input of a row, col index and then computes five coordinates based on where that element is on a map. four corners and the center of the element
def ERA5_five_coordinate_finder(row, col):
    resolution = 1 / 4
    global row_coord
    global col_coord
    global shifted_row_coord
    ...
    if row_coord < 0 and shifted_row_coord > 0:
        temp_row_coord = row_coord + 180
    center_row_coord = temp_row_coord - resolution / 2
    row_coord = truncate(row_coord,4)

    #col determining lines of code
    col_coord = (float(col * resolution)) % 360
    shifted_col_coord = (float(col * resolution) + resolution) % 360
    center_col_coord = col_coord + resolution / 2
    col_coord = truncate(col_coord,4)
...
#This subroutine will convert a coordinate (lat,long) into index values based on an ETOPO5 file (2160,4320) -90 <= lat <= 90 and 0 <= long <= 360
def ERA5_index_finder(lat,long):
    resolution = 1 / 4

    global row_index
    global col_index
    row_index = (abs(int(round(float(lat) / resolution)) - 360))# dividing by resolution is same as multiplying by reciprical
    col_index = int(round(float(long) / resolution))

    #print("Indices for coordinate (" + str(lat) + ', ' + str(long) + ') are (' + str(row_index) + ', ' + str(col_index) +')')
    print("Index found")
    return row_index, col_index

def twelfth_to_fourth_rounder(remainder): #changes any remainder from the lat/long to how many more indices the values should change based on the decimal remainder
    remainder = truncate(remainder,4)
    if remainder == 0.4999 or remainder == 0.2499 or remainder == 0.9999 or remainder == 0.7499:
        remainder += 0.0001
    print(remainder)
    switcher = {
        0.0000: 0,
    ...
    return switcher.get(remainder)

#This subroutine does the same thing as the other ERA-5 index finder except that it is for a smaller range (51,73)
def ERA5_index_finder_QuikSCAT_region(lat,long):
    resolution = 1 / 4
    # 2000 km plot limits
    rowBegin = 541
    rowEnd = 613
    ...
    global row_index
    global col_index
    row_remainder_val_index = twelfth_to_fourth_rounder(float(lat) % 1) #accounts for the shift in indices for whatever the decimal of the lat/long is
    col_remainder_val_index = twelfth_to_fourth_rounder(float(long) % 1)
    row_index = (int((90 - int(float(lat))) / resolution ) - rowBegin) - row_remainder_val_index + 4 # dividing by resolution is same as multiplying by reciprical, subtracts from 556 because then the index value fits between the (51,73) range
    col_index = int(((360 - int(float(long))) / resolution) - colBegin) - col_remainder_val_index   #
    print("Indices for coordinate (" + str(lat) + ', ' + str(long) + ') are (' + str(row_index) + ', ' + str(col_index) +')')
    print()
    print()
    print()
...
def ERA5_index_finder_2000_km_plot_region(lat,long):
    resolution = 1/4
    row_remainder_val_index = twelfth_to_fourth_rounder(float(lat) % 1)  # accounts for the shift in indices for whatever the decimal of the lat/long is
    col_remainder_val_index = twelfth_to_fourth_rounder(float(long) % 1)
    row_index = int(abs(float(lat) + 45.3) / resolution)  # dividing by resolution is same as multiplying by reciprocal
    #print(long)
    col_index = int(abs((360 - float(long)) - 307.5) / resolution)
    return row_index, col_index

def Two_thousand_km_index_finder(lat,long):
    ...
    row_index = int(abs(float(lat) + 45.3) / lat_resolution)# dividing by resolution is same as multiplying by reciprical
    #print(long)
    col_index = int(abs(float(long) - 307.5) / long_resolution)
...
def Two_thousand_km_index_finder_from_ETOPO5(lat,long): #converts ETOPO5 coordinates to 2000 km plot index
    lat_resolution = 10/111.111
    long_resolution = (10 / (111.111 * np.cos(np.deg2rad(54.3))))
    ...
    row_index = int(abs(float(lat) + 45.3) / lat_resolution)# dividing by resolution is same as multiplying by reciprical
    #print(long)
    col_index = int(abs((360 - float(long)) - 307.5) / long_resolution)
